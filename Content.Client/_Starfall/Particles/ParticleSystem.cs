...
using Robust.Shared.Physics.Components; // stalker-en
...
public sealed partial class ParticleSystem : EntitySystem
{
    ...
    public ActiveEmitter? SpawnEffect(ProtoId<ParticleEffectPrototype> effectId, MapCoordinates coords, EntityUid? attachedEntity = null, Color? colorOverride = null,
        ParticleRuntimeOverrides? overrides = null) // stalker-en
    {
        ...
        emitter.Overrides = overrides; // stalker-en

        if (proto.Burst)
        { // stalker-en-start - burst bypasses velocity which lives in the tick loop so it has to be done this way
    ...
    {
        // stalker-en-start - added since burst bypasses angle override checks that are found in the tick update
        var baseAngle = emitter.Overrides?.EmitAngle ?? emitter.Proto.EmitAngle;
        emitter.EffectiveEmitAngle = (float)baseAngle.Theta;
        // stalker-en-end
